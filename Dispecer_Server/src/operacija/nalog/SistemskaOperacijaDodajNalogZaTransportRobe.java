...
public class SistemskaOperacijaDodajNalogZaTransportRobe extends ApstraktnaSistemskaOperacija {
    ...
    protected void validiraj(ApstraktniDomenskiObjekat ado) throws Exception {
        ...
        if (noviNalog.getDatumKreiranja() == null) {
            throw new Exception("Datum kreiranja nije unet!!");
        }

        if (noviNalog.getDatumKreiranja().before(danasnjiDatum)) {
            throw new Exception("Datum kreiranja mora biti u buducnosti!!");
        }
        ...
        ArrayList<NalogZaTransportRobe> nalozi = (ArrayList<NalogZaTransportRobe>) (ArrayList<?>) DBBroker.getInstance().vratiSve(ado);
